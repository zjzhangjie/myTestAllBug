  ...
  !(function(t) {
      ...
      t.exports = function(t) {
        (t._get_linked_task = function(e, i) {
          ...
          return (
            t.assert(
              n,
              "Link " + a + " not found. Task id=" + r + ", link id=" + e.id
            ),
            n
          );
        }),
          ...
          (t._formatLink = function(e) {
            ...
            if (!r || !n) return i;
            if (
              (t.isChildOf(r.id, n.id) && t.isSummaryTask(n)) ||
              (t.isChildOf(n.id, r.id) && t.isSummaryTask(r))
            )
              return i;
            for (
              var a = this._getImplicitLinks(e, r, function(t) {
                  return 0;
                }),
                s = t.config.auto_scheduling_move_projects,
                c = this.isSummaryTask(n)
                  ? this.getSubtaskDates(n.id)
          ...
          (t._getImplicitLinks = function(t, e, i) {
            var n = [];
            return (
              this.isSummaryTask(e)
                ? this.eachTask(function(t) {
                    this.isSummaryTask(t) || n.push({ task: t.id, lag: i(t) });
                  }, e.id)
                : n.push({ task: e.id, lag: 0 }),
              n
            );
          }),
          (t._getDirectDependencies = function(t, e) {
            for (
              var i = [], n = [], r = e ? t.$source : t.$target, a = 0;
              a < r.length;
              a++
            ) {
            ...
            for (var a = 0; a < i.length; a++)
              n = n.concat(this._formatLink(i[a]));
            return n;
          }),
                  ...
                  n ||
                    (this.isSummaryTask(t) &&
                      (this._isAutoSchedulable(t)
                        ? r.push.apply(r, this._getDirectDependencies(t, e))
                        : (n = !0)));
                },
                t.id,
                this
          ...
          (t._getSuccessors = function(t) {
            return this._getDirectSuccessors(t).concat(
              this._getInheritedSuccessors(t)
            );
          }),
          (t._getPredecessors = function(t) {
            return this._getDirectPredecessors(t).concat(
              this._getInheritedPredecessors(t)
            );
          }),
    ...
    function(e, i, n) {
      n(1)(t),
        ...
        (t._isCriticalTask = function(t, e) {
          if (t && t.id) {
            var i = e || {};
            ...
            for (var n = this._getDependencies(t), r = 0; r < n.length; r++) {
              ...
              if (
              //this._getSlack(t, a, n[r]) <= 0 &&  //目前处理情况，后续需要调价延时或者提前相关时间
              !i[a.id] &&
                this._isCriticalTask(a, i)
              )
                return !0;
            }
            return !1;
        ...
        (t.getSlack = function(t, e) {
          for (var i = [], n = {}, r = 0; r < t.$source.length; r++)
            n[t.$source[r]] = !0;
          for (var r = 0; r < e.$target.length; r++)
            n[e.$target[r]] && i.push(e.$target[r]);
          for (var a = [], r = 0; r < i.length; r++) {
            ...
            a.push(
              this._getSlack(
                t,
                e,
                this._convertToFinishToStartLink(s.id, s, t, e)
              )
            );
          }
        ...
        (t._getSlack = function(t, e, i) {
          var n = this.config.types,
            r = null;
          r =
            this.getTaskType(t.type) == n.milestone ? t.start_date : t.end_date;
          var a = e.start_date,
            s = 0;
          s =
            +r > +a
              ? -this.calculateDuration({ start_date: a, end_date: r, task: t })
              : this.calculateDuration({ start_date: r, end_date: a, task: t });
          var c = i.lag;
