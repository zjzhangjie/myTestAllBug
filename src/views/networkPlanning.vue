    ...
    export default {
        ...
        methods:{
            ...
            setPosition(i,item){
                ...
                let y=2*cardSize+30+(30*i);//距离顶有30的高度
                let r=10;
                if(item.link==0){//不存在前置任务
                    ...
                    //中间的线
                    vm.drawLines(cardSize,r,i,item);
                }else{//存在前置任务
                    ...
                    if(item.link_type=='SS'){
                        ...
                        let color="red";
                        vm.drawLines(cardSize,r,i,item,color);//画实线
            ...
            //中间的实线和箭头
            drawLines(cardSize,r,i,item,color){
                let canvas=document.getElementById('myCanvas');
                let ctx=canvas.getContext('2d');
                let x_start=item.start_time*cardSize-cardSize;
                let x_end=item.end_time*cardSize;
                ...
                ctx.setLineDash([0,0]);
                ctx.beginPath();
                ctx.moveTo(x_start+r,y);//这里的8指的是圆圈的半径
